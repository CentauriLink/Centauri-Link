...
class MJPEGStreamer:
    ...
    def _run(self, fps_limit):
        url = f"http://{self._ip}:3031/video"
        try:
            ...
            for chunk in r.iter_content(chunk_size=1024):
                ...
                if a != -1 and b != -1 and b > a:
                    ...
                    buffer = buffer[b + 2:]
                    now = time.time()
                    if now - last_frame < (1.0 / fps_limit):
                        continue
                    last_frame = now
...
class HomeScreen(Screen):
    def start_camera(self):
        ...
        if img:
            ...
            app._set_status("Camera started")
        else:
    ...
    def toggle_auto_poll(self, enabled: bool):
        ...
        if self.auto_poll:
            if not app.ws_running:
                ...
                self._poll_event = Clock.schedule_interval(lambda dt: threading.Thread(target=app._http_fallback_fetch, daemon=True).start(), POLL_INTERVAL)
...
class CentauriApp(App):
    ...
    def _start_ws(self, host: str):
        ...
        self.ws_app = websocket.WebSocketApp(url, on_message=on_message, on_error=on_error, on_close=on_close, on_open=on_open)
    ...
    def _reliable_send(self, fan_key: str, speed: int):
        ...
        if custom:
            try:
                txt = custom.replace("{fan_key}", fan_key).replace("{speed}", str(int(speed)))
                try:
        ...
        if self.mainboard_id:
            candidates.append({"Topic": f"sdcp/command/{self.mainboard_id}", "Cmd": "SetFanSpeed", "Data": {fan_key: int(speed)}})
        candidates.append({"cmd": "set_fan", "data": {fan_key: int(speed)}})
        candidates.append({"action": "setFanSpeed", "params": {fan_key: int(speed)}})
